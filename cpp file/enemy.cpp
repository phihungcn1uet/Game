...
Enemy :: Enemy() {
	...
	mTextureBoss = NULL;
	BossBox = { 0,0,0,0 };
	mTextureBossSkill = NULL;
	BossBoxSkill = { 0,0,0,0 };
	bossdeath = false;
}
Enemy :: ~Enemy() {
	free();
...
bool Enemy::loadFromFileBossSkill(std::string path, SDL_Renderer* screen) {
	free2();
	SDL_Texture* newTexture = NULL;
	...
	else
	{
		...
		{
			BossBoxSkill.w = loadedSurface->w;
			BossBoxSkill.h = loadedSurface->h;
		}
		SDL_FreeSurface(loadedSurface);
	}
	mTextureBossSkill = newTexture;
	return mTextureBossSkill != NULL;
}
...
void Enemy::timerandomboss(const int& a) {
	...
	int currenttime = 0;
	if (du < 30) {
		if (time % TimeranBoss == 0 && time != 0) {
			int currenttime = time;
			int x_pos = 1280;
			int y_pos = rand() % (250 - 150 + 1) + 150;
...
void Enemy::bossmove(SDL_Renderer* screen, std::vector <Bullet>& bullets, bool& death, const bool& bullettype, character& character, const bool& collision,const int &a,TTF_Font * font_hp) {
	...
	for (int i = 0; i < boss1.size(); i++) {
		...
		if (skill1.size() > 0) {
			for (int k = 0; k < skill1.size(); k++) {
				...
				BossBoxSkill.x = skill1[k].x+10;
				BossBoxSkill.y = skill1[k].y + 15;
				SDL_RenderCopy(screen, mTextureBossSkill, NULL, &BossBoxSkill);
				if (collision) {
					if (checkCollision(character.mBox, BossBoxSkill)) {
						death = true;
					}
				}
				for (int j = 0; j < bullets.size(); j++) {
					SDL_Rect a = { bullets[j].x,bullets[j].y,32,32 };
					if (checkCollision(a, BossBoxSkill)) {
							bullets.erase(bullets.begin() + j);
					}
				}
				if (skill1[k].x < 0) {
			...
			//Neu dan ban trung boss
			if (bullets.size() != 0) {
				for (int j = 0; j < bullets.size(); j++) {
					SDL_Rect a = { bullets[j].x,bullets[j].y,32,32 };
					SDL_Rect b = { boss1[i].x,boss1[i].y,BossBox.w,BossBox.h };
					if (checkCollision(a, b) && bullettype==true) {
						bullets.erase(bullets.begin() + j);
						HP--;
					}
					else if (checkCollision(a, b) && bullettype == false) {
						bullets.erase(bullets.begin() + j);
						HP-=2;
					}
					if (HP == 0) {
...
void Enemy::free1() {
	if (mTextureBoss != NULL)
	{
		SDL_DestroyTexture(mTextureBoss);
		mTextureBoss = NULL;
		BossBox = { 0,0,0,0 };
	}
}
...
void Enemy::free2() {
	if (mTextureBossSkill != NULL)
	{
		SDL_DestroyTexture(mTextureBossSkill);
		mTextureBossSkill = NULL;
		BossBoxSkill = { 0,0,0,0 };
	}
}
